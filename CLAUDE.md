...
For restart commands that need `sudo`: never run them yourself. List them for Eduardo to run, clearly marked as his to execute.
...
STOP. Name the ambiguity in one sentence. Present 2-3 options with real trade-offs (not a fake spread). Ask Eduardo. Do not guess on architectural decisions. Does not apply to routine coding, small features, or obvious changes.
